...
- `admin/js/ai.js` + two UI panels in `app.js` are the CMS features with a real server component — `oauth-worker/worker.js` proxies both to the Anthropic Messages API using a server-side `ANTHROPIC_API_KEY` secret (never exposed to the browser) via a shared `requirePushAccess()` check that rejects any caller whose GitHub token doesn't have push access to `GITHUB_REPO` (another worker secret) — without that check, anyone who found the worker's URL could spend the site owner's API credits with a throwaway GitHub account.
