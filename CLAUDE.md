...
- The `image_position` field type (`fieldInputHtml`'s `"image_position"` case, `cropFor` pointing at the paired `image` field name) renders a 3×3 focus-point picker over a live preview of that image, storing a CSS `object-position` value (e.g. `"center top"`) — `wireCropFields()` handles clicks and `wireImageFields()`/`updateCropPreview()` keep the preview's background in sync when a new photo is uploaded. `_includes/story-card.html`, `_layouts/post.html`, and `_layouts/home.html` (the lead/featured story) all read `post.image_position` (default `"center center"`) as an inline style — this is additive, so posts saved before this field existed still render unchanged.
